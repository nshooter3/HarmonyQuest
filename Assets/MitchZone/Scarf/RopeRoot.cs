...
public class RopeRoot : MonoBehaviour {
    ...
    private void AddChildren(Transform parent)
    {
        for (int i = 0; i < parent.childCount; i++)
        {
            ...
            representative.transform.position = child.position;
            //rigidbody
            ...
            // Verlet
            var verlet = representative.gameObject.AddComponent<VerletConstraint>();
            verlet.t2 = parent;
            verlet.compensate1 = 8f;
            verlet.distance = Vector3.Distance(parent.position, child.position);

            // Friction
            // var friction = representative.gameObject.AddComponent<FrictionJoint3D>();
